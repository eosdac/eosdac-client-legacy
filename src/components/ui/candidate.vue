...
<div>
  ...
  <div class="q-mb-md relative-position bg-bg1 bg-logo round-borders lt-sm"  v-bind:class="{ 'selected_candidate': data.selected, 'unselected_candidate':!data.selected, 'shadow-5':!data.selected}">
    ...
    <div class="row justify-center q-pt-md">
      <div class="profile_image animate-fade"  style="width: 100px; height:100px" v-bind:style="{ 'background-image': 'url(' + profileImage + ')' }" ></div>
    </div>

    <div class="row justify-center">
      <router-link class="q-headline" :to="{path: '/profile/' + data.candidate_name}" >
        <q-icon :title="$t('candidate.nominated_next')" style="margin-top:-5px" v-if="data.rank < 13" name="star_border" color="yellow" />{{ data.candidate_name }}
      </router-link>
    </div>
    ...
    <q-modal maximized v-model="profilemodal">
      <div style="height:50px" class="bg-bg2 row items-center justify-between q-px-md text-text1">
        <span>Bio</span>
        <q-btn icon="icon-ui-8" @click="profilemodal = false" class="no-shadow"/>
      </div>
      <div class="q-pa-md bg-bg1 full-height">
        <div class="row items-center q-mb-md">
          <div class="profile_image on-left animate-fade"  style="width: 60px; height:60px" v-bind:style="{ 'background-image': 'url(' + profileImage + ')' }" ></div>
          <router-link class="q-headline" :to="{path: '/profile/' + data.candidate_name}" >
            <q-icon :title="$t('candidate.nominated_next')" style="margin-top:-5px" v-if="data.rank < 13" name="star_border" color="yellow" />{{ data.candidate_name }}
          </router-link>
        </div>
      ...
      <template slot="header" >
        <q-item-side left >
          <div class="row full-height items-center">
            ...
            <div class="profile_image float-left animate-fade"  style="width: 60px; height:60px" v-bind:style="{ 'background-image': 'url(' + profileImage + ')' }" ></div>
          </div>
        </q-item-side>
        <q-item-main >
          <div class="q-ml-lg">
            <router-link class="q-headline" :to="{path: '/profile/' + data.candidate_name}" >
              <q-icon title="Nominated for next custodian board" style="margin-top:-5px" v-if="data.rank < 13" name="star_border" color="yellow" />{{ data.candidate_name }}
            </router-link>
            <span class="text-text2"  v-if="data.profile && (data.profile.givenName !='' || data.profile.familyName !='')" >({{data.profile.givenName}} {{data.profile.familyName}})</span>
            <br>
            <span><span class="text-text2">{{$t('candidate.votes')}}:</span> {{ $helper.toLocaleNumber(data.total_votes/10000) }}</span>
