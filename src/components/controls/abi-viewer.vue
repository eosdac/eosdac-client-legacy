<template>
    ...
    <div>
      <div class="q-mb-sm">
        <q-item>
          <q-item-main>
            <q-input :dark="getIsDark" v-model="contract" type="text" stack-label="Contract" placeholder="input a contract name" @keyup.enter.native="getAbi(contract)" />
          </q-item-main>
          <q-item-side right>
            <q-btn color="primary-light"  flat label="load" @click="getAbi(contract)" />
          </q-item-side>
        </q-item>
      </div>
      
      <div v-if="abi.version" class="row q-mb-md bg-bg2 q-pa-xs" >
        <q-btn size="10px" @click="$emit('ActionSelected', {account: contract, name: action.name})" :icon="$configFile.icon.action" v-for="(action, i) in abi.actions" :label="action.name" :key="`a${i}`" color="bg1" class="animate-pop q-ma-xs"  />
      ...
      <div v-if="abi_load_error!=''" class="text-negative q-caption">{{abi_load_error}}</div>
