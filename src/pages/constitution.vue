...
<div class="row gutter-md reverse-wrap">
  ...
  <div class="col-xs-12 col-xl-8" >
      ...
      <div class=" bg-bg1 shadow-5 round-borders">
          ...
          <div class="row justify-end bg-bg1 q-pa-md round-borders">
            <q-toggle color="primary-light" v-model="white_constitution" class="text-text1" left-label :label="$t('constitution.switch_contrast')" />
          </div>
          <div  class="q-pa-md bg-bg1 full-width" style="min-height:1000px">
            <div  v-if="constitution !=''" v-bind:class="{ overwrite: !white_constitution }" class="markdown-body testtt animate-fade" v-html="constitution"></div>
          </div>
  ...
  <div class="col-xs-12 col-xl-4 text-text1" >
    <div>
      <div id="registerbox" class="bg-logo bg-bg1 q-pa-md round-borders shadow-5 relative-position" style="overflow:hidden">
          ...
          <q-item>
            ...
            <q-item-main style="overflow:hidden">
              ...
              <q-item-tile class="text-text2" style="overflow:hidden; white-space:nowrap;" sublabel>
                <span ><a  target="_blank" :href="getLatestMemberTerm.terms">{{getLatestMemberTerm.terms}}</a></span>
              </q-item-tile>
            </q-item-main>
          </q-item>
          ...
          <div class="row justify-end items-center">
            <q-btn v-if="needSignature"  @click="signConstitution()"  color="primary" label="register" />
            <div  v-if="!needSignature" class="text-positive q-caption q-my-sm" >{{ $t('constitution.signed_message') }}</div>
            <q-btn v-if="!needSignature" class="on-right" @click="unRegister()"  color="primary" label="unregister" />
          </div>
...
export default {
  ...
  methods: {
    ...
    async getConstitution() {
      ...
      this.isloading = true;
      try {
        let getCt = await this.loadConstitutionFromGithub(this.getLatestMemberTerm.terms);
        ...
        this.constitution = '<span class="animate-fade">'+marked(getCt, {sanitize: true})+'</span>';
    ...
    async signConstitution() {
      ...
      if(result){
        this.$store.commit('user/setAgreedTermsVersion', this.getLatestMemberTerm.version );
      }
