  ...
  <q-page class="relative-position" style="overflow-x: hidden; ">
    ...
    >
      <div class="row">
        <div class="col-12">
          <q-btn
            v-if="!profile_is_irrevirsible"
            @click="initProfile"
            class="float-right"
            color="red"
            :label="$t('profile.not_confirmed')"
          />
      ...
      <div class="animate-pop profile_image_outer_wrap" style>
        ...
        <div class="fit profile_image_inner_wrap">
          <transition name="fade">
            <img
              :class="{
                hack_center: centerimage,
                'full-width': setwidth,
                'full-height': !setwidth
              }"
              v-bind:src="setImgSrc"
              v-on:load="onLoaded"
              v-show="loaded"
              ref="profile_pic"
            />
      ...
      <div
        class="blur-details q-pa-md absolute-bottom"
        style="height:120px;margin-right:-16px;margin-left:-16px;"
      >
    ...
    <div class="q-px-md">
      <div
        class="row q-mt-md gutters-md bg-bg1 round-corners shadow-5"
...
export default {
  ...
  methods: {
    ...
    onLoaded() {
      ...
      this.setwidth = img.width <= img.height;
      this.centerimage = img.width !== img.height;
      this.visible = false;
      this.loaded = true;
    },
    //init profile page
    ...
    async getProfileData() {
      ...
      if (p && p.length && this.validateProfile(p[0].profile)) {
        ...
        this.allow_edit =
          this.account_name === this.getAccountName &&
          this.profile_is_irrevirsible;
      } else {
  ...
  watch: {
    ...
    getAccountName: function(val) {
      this.allow_edit =
        this.account_name === val && this.profile_is_irrevirsible;
    }
  }
};
</script>
