...
<!-- is already a candidate UNREGISTER-->
  <div v-if="getIsCandidate && getIsCandidate.is_active" class=" bg-logo bg-bg1 q-pa-md round-corners shadow-5 relative-position overflow-hidden">
    <!-- <div class="" style="bottom:-50%;right:-80%;"></div> -->
    <q-item class="no-padding">
      <q-item-side left>
        <div style="width:80px;height:80px" class="profile_image animate-pop" v-bind:style="{ 'background-image': `url(${getProfilePicture})` }"></div>
      </q-item-side>
      <q-item-main>
        <q-item-tile class="text-text1 q-display-1" label>{{$t('regcandidate.candidate')}}</q-item-tile>
        <q-item-tile class="text-text2 q-title" sublabel>
          <div class="row items-center">{{getIsCandidate.candidate_name}}<q-icon name="check" color="positive" /></div>
        </q-item-tile>
       
      </q-item-main>
    </q-item>
    <div class="q-mt-md">{{$t('regcandidate.page_description_registered')}}</div>
    ...
    <div class="row q-mt-md">
      <q-item class="q-pl-none">
        ...
        <q-item-main style="margin-left:-5px">
          <q-item-tile class="text-text1" label>{{$t(`regcandidate.requestedpay`)}}</q-item-tile>
          <q-item-tile class="text-text2" sublabel>{{ $helper.assetToLocaleNumber(getIsCandidate.requestedpay) }}</q-item-tile>
        </q-item-main>
      </q-item>
      <q-item class="q-pl-none">
        <q-item-side left icon="icon-dac-balance" class="text-text2" />
        <q-item-main style="margin-left:-5px">
          <q-item-tile class="text-text1" label>{{ $t(`regcandidate.locked_tokens`) }}</q-item-tile>
          <q-item-tile class="text-text2" sublabel>{{ $helper.assetToLocaleNumber(getIsCandidate.locked_tokens) }}</q-item-tile>
        </q-item-main>
      </q-item>
      <q-item class="q-pl-none">
        <q-item-side left icon="how_to_vote" class="text-text2" />
        <q-item-main style="margin-left:-5px">
          <q-item-tile class="text-text1" label>{{$t(`regcandidate.total_votes`)}}</q-item-tile>
          <q-item-tile class="text-text2" sublabel>{{ $helper.toLocaleNumber(getIsCandidate.total_votes/10000) }}</q-item-tile>
        </q-item-main>
      </q-item>
    </div>
    <div class="row justify-end">
      <q-btn class="animate-pop" color="primary" @click="unRegisterAsCandidtate" :label="$t('regcandidate.unregister')" />
    </div>
  ...
  <div v-else class="bg-bg1 bg-logo q-pa-md round-corners shadow-5">
    ...
    <div class="row gutter-md q-mt-md">
      <div class="col-xs-12 col-md-6">
        <div>
          <span>{{$t('regcandidate.stake_description', {minimum_stake: $helper.assetToLocaleNumber(getCustodianConfig.lockupasset) }) }}</span>
          <q-item class="q-pl-none q-mb-md">
            <q-item-side left icon="icon-dac-balance" v-bind:class="{'text-positive':verifyAndGetStakeAmount, 'text-text2': !verifyAndGetStakeAmount}"/>
            <q-item-main>
              <q-input  color="primary-light"  :dark="getIsDark" type="number" v-model="inputs.stakeamount" :stack-label="$t('regcandidate.stake_amount')" :placeholder="$t('regcandidate.amount_to_stake_placeholder')" />
            </q-item-main>
          </q-item>
        </div>
      </div>
      <div class="col-xs-12 col-md-6">
        <div>
          ...
          <q-item class="q-pl-none">
            ...
            <q-item-main>
              <q-input  color="primary-light" :dark="getIsDark" type="number" :max="20" v-model="inputs.requestedpay" :stack-label="$t('regcandidate.requestedpay')" :placeholder="$t('regcandidate.requested_custodian_pay_placeholder')" />
            </q-item-main>
          </q-item>
        </div>
      </div>
    ...
    <div class="row justify-end q-mt-md">
      <q-btn :disabled="!verifyAndGetRequestedPay || !verifyAndGetStakeAmount" class="animate-pop" color="primary" @click="registerAsCandidtate" :label="$t('regcandidate.register')" />
    </div>
  </div>
<!-- end not a candidate -->
