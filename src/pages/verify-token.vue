...
<q-page class="text-text2 q-pa-md">
  ...
  <div v-if="enable_gui" class="q-pa-md bg-bg1 bg-logo shadow-5 round-borders">
    <p class="text-text2">{{$t('verify_token.description')}}</p>
    <div class="row justify-start full-width">
      <div style="display:inline-block;" class="bg-primary q-pa-sm">{{$t('verify_token.token')}}:</div> 
      <div  style="width:50%;max-width:500px;display:inline-block;"  class="bg-bg2 q-pa-sm"><div style="overflow:hidden">{{token}}</div></div>
      <div class="bg-bg2 q-pa-sm cursor-pointer" style="display:inline-block" @click="copyToClipboard"> <q-icon style="margin-top:-3px" name="file_copy" /> </div>
      <span v-if="copied_to_clipboard_msg !=null" class="q-pa-sm animate-fade q-caption text-text2" >{{copied_to_clipboard_msg}}</span>
    </div>
  ...
  <div v-if="!enable_gui" class="q-pa-md bg-bg1 shadow-5 round-borders text-text2">
    Please login <span class="text-bold text-text1">{{actor}}</span> to verify your token.
  </div>
